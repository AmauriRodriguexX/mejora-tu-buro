...
{#if currentRoute === '/'}<div class:theme-dark={theme === 'dark'} class="site"><a class="skip-link" href="#contenido">Ir al contenido</a><Navbar {theme} route="/" /><main id="contenido"><Hero {savingPlan} /><Opiniones /><TrustBanner /><HowItWorks /><Calculator bind:savingPlan /><Guarantees /><Faq /><BlogPreview /></main><Footer bind:theme /><MobileBottomNav /><CookieBanner /></div>{:else}<RoutePage route={currentRoute} />{/if}
