...
<section id="blog" class="surface py-16 sm:py-20" aria-labelledby="blog-title">
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
      <div class="max-w-2xl">
        <p class="eyebrow">Blog Mejora Buró</p>
        <h2 id="blog-title" class="mt-3 text-3xl font-extrabold sm:text-4xl">Información para tomar mejores decisiones</h2>
        <p class="muted mt-4 leading-relaxed">Guías sencillas para entender tus deudas, tus opciones y los siguientes pasos.</p>
      </div>
      <a class="btn-secondary focus-ring shrink-0" href={`${base}/entradas/`}><span class="button-label">Ver todo el blog <span aria-hidden="true">→</span></span></a>
    </div>
    <div class="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{#each posts as post}<a class="blog-card card rounded-2xl p-5" href={`${base}/entradas/${post.categorySlug}/${post.slug}/`} aria-label={`Leer artículo: ${post.title}`}><img class="blog-card-image" src={`${base}/images/blog/${post.image}`} alt="" loading="lazy" /><div class="mt-5 flex items-center justify-between gap-3 text-xs font-bold uppercase tracking-wide"><span style="color:var(--brand)">{post.category}</span><time class="muted datetime" datetime={`${post.date}-01-01`}>{post.date}</time></div><h3 class="mt-3 text-xl font-bold leading-snug">{post.title}</h3><p class="muted mt-3 text-sm leading-relaxed">{post.excerpt}</p><span class="footer-link mt-5 inline-flex text-sm font-bold">Leer artículo <span aria-hidden="true">→</span></span></a>{/each}</div>
  </div>
</section>
